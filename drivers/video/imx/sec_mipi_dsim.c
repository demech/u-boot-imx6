...
struct dsim_clk_priv {
	struct clk	clk;
	unsigned long	frequency;
};

static unsigned long dsim_clk_recalc_rate(struct clk *clk)
{
	struct dsim_clk_priv *clk_priv = container_of(dev_get_clk_ptr(clk->dev),
			struct dsim_clk_priv, clk);
	return clk_priv->frequency;
}

static unsigned long dsim_clk_set_rate(struct clk *clk, unsigned long rate)
{
	struct dsim_clk_priv *clk_priv = container_of(dev_get_clk_ptr(clk->dev),
			struct dsim_clk_priv, clk);
...
static int dsim_clk_probe(struct udevice *dev)
{
	struct dsim_clk_priv *clk_priv = dev_get_priv(dev);
	...
	if (name)
		clk_name = name;
	name = dev_read_string(dev, "clock-pllref-name");
	if (name)
		clk_name = clk_parent_name;

	/* register the clock */
	return clk_register(&clk_priv->clk, "sec_mipi_dsim-clk", clk_name, clk_parent_name);
}

static struct clk_ops dsim_clk_ops = {
	.set_rate = dsim_clk_set_rate,
	.get_rate = dsim_clk_recalc_rate,
};

static const struct udevice_id sec_mipi_dsim_clk_ids[] = {
	{ .compatible = "samsung,sec-mipi-dsi-clk" },
	{ }
};

U_BOOT_DRIVER(imx8mp_clk) = {
	.name = "sec_mipi_dsim-clk",
	.id = UCLASS_CLK,
	.of_match		= sec_mipi_dsim_clk_ids,
	.ops = &dsim_clk_ops,
	.probe = dsim_clk_probe,
	.priv_auto_alloc_size = sizeof(struct dsim_clk_priv),
};
